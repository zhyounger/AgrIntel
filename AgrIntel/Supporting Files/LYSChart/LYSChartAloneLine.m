...
@implementation LYSChartAloneLine
...
- (void)drawLineChart
{
    ...
    LYSAnimationLayer *animationLayerLine = [self lineChartWith:self.canvasLeftBottomPoint
    ...
    animationLayerLine.strokeEnd = 0;
    [self.layer addSublayer:animationLayerLine];
    [_chartLayerAry addObject:animationLayerLine];
    
    UIBezierPath *path = [UIBezierPath bezierPath];
    if (_isCurve == NO) {
        [path moveToPoint:POINT(0, [_valueData[0] floatValue] * scale + _lineChartWidth)];
        for (int i = 0; i < [_valueData count]; i++) {
            [path addLineToPoint:POINT(self.columnSpace * i, [_valueData[i] floatValue] * scale)];
        }
    }else{
        [path moveToPoint:POINT(0, [_valueData[0] floatValue] * scale)];
        for (int i = 0; i < [_valueData count]; i++) {
            if (i + 1 < [_valueData count]) {
                CGPoint point1 = CGPointMake(self.columnSpace * i + self.columnSpace * 0.5,[_valueData[i] floatValue] * scale);
                CGPoint point2 = CGPointMake(self.columnSpace * i + self.columnSpace * 0.5,[_valueData[i + 1] floatValue] * scale);
                [path addCurveToPoint:POINT(self.columnSpace * (i + 1), [_valueData[i + 1] floatValue] * scale) controlPoint1:point1 controlPoint2:point2];
            }
        }
    }
    ...
    [animationLayerLine animationSetPath:^CGPathRef(CADisplayLink *displayLink) {
        ...
        animationLayerLine.strokeEnd += 0.01;
        
        if (animationLayerLine.strokeEnd > 1) {
            ...
            for (int i = 0; i < [_valueData count]; i++) {
                ...
                CATextLayer *ytext = [self textLayer:POINT(self.columnSpace * i, [_valueData[i] floatValue] * scale + 2) text:_valueData[i] fontColor:_yValueDataStyle.fontColor fontSize:_yValueDataStyle.fontSize boxSize:CGSizeMake(_edgeInsets.left, rect.size.height)];
                ytext.anchorPoint = CGPointMake(0.5, 0);
                ytext.alignmentMode = kCAAlignmentCenter;
                [animationLayerLine addSublayer:ytext];
                
                CAShapeLayer *dot = [self dotWith:POINT(self.columnSpace * i, [_valueData[i] floatValue] * scale) radius:_lineChartDotRadius color:_lineChartColor];
